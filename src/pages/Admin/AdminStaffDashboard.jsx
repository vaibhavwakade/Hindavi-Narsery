...
const AdminStaffDashboard = ({ userId }) => {
    ...
    const fetchSummary = async () => {
      try {
        ...
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/staff/self-summary/${userId}`,
          {
  ...
  if (!summary) {
    return (
      <div className="text-center text-green-600 mt-10">No data available.</div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 mt-8 bg-white shadow-lg rounded-lg border border-green-200">
      ...
      <div className="mb-4 space-y-1">
        ...
        <p className="text-green-800">
          <strong>Salary:</strong> ₹{summary.salary}
        </p>
      </div>
            ...
            {filteredAttendance.length > 0 ? (
              filteredAttendance.map((a, i) => (
                <tr key={i} className="hover:bg-green-50">
                  ...
                  <td className="py-2 px-4 border border-green-200 capitalize">
                    {a.status}
                  </td>
                </tr>
              ))

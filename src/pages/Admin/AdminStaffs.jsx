  ...
  const fetchStaffAttendance = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/staff/attendance/`,
        {
  ...
  const handleSalaryUpdate = async (userId) => {
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BACKEND_URL}/staff/salary/${userId}`,
        { salary: editedSalary },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      ...
      setStaff((prev) =>
        prev.map((s) =>
          s.userId === userId ? { ...s, salary: editedSalary } : s,
        ),
      );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center mb-8">
          ...
          <span className="ml-3 px-3 py-1 bg-green-100 text-green-800 text-sm font-medium rounded-full">
            ...
            {filteredStaff.length === 1 ? "Staff" : "Staffs"}
          </span>
        </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  {selectedDate >= new Date().toISOString().split("T")[0] && (
                    <select
                      value={s.status}
                      onChange={(e) =>
                        handleAttendanceChange(s.userId, e.target.value)
                      }

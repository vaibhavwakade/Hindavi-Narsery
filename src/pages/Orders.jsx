...
function Orders() {
  ...
  const filterOrders = () => {
    ...
    if (dateFilter !== "all") {
      ...
      filtered = filtered.filter(
        (order) => order?.createdAt && new Date(order.createdAt) >= filterDate,
      );
    }
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response) {
          try {
            await axios.put(
              `${import.meta.env.VITE_BACKEND_URL}/orders/pay/${order.id}`,
              {},
              { headers: { Authorization: `Bearer ${token}` } },
            );
  ...
  const confirmQrCodePayment = async (orderId) => {
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_BACKEND_URL}/orders/pay/${orderId}`,
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium ${order.paymentStatus === "paid" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                  >
                ...
                <div className="p-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-sm font-medium ${order.paymentStatus === "paid" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
                      >
                        {order.paymentStatus.charAt(0).toUpperCase() +
                          order.paymentStatus.slice(1)}
                      </span>
                  ...
                  <div className="flex flex-wrap gap-3 pt-4 border-t">
                    ...
                    {order.status === "delivered" && (
                      <button
                        onClick={() =>
                          navigate(
                            `/products/${order.items[0].product.id}/review`,
                          )
                        }

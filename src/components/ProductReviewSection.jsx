...
function ProductReviewSection({ productId }) {
    ...
    const fetchReviews = async (skipFormStateUpdate = false) => {
        try {
            ...
            const currentUserReview = allReviews.find(r => r.user?._id === res.data.userId);
    ...
    const checkPermissions = async () => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/reviews/can-review/${productId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (editingReviewId) {
                await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/reviews/${editingReviewId}`, { rating, message }, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                toast.success('Review updated');
            } else {
                await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/reviews/${productId}`, { rating, message }, {
                    headers: { Authorization: `Bearer ${token}` },
                });
    ...
    const handleDelete = async (reviewId) => {
        try {
            await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/reviews/${reviewId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    return (
        <div className="mt-12">
            <h2 className="text-2xl font-semibold text-green-800 mb-6">Customer Reviews</h2>

            {(canReview || editingReviewId) && (
                <div className="mb-8 p-4 bg-green-50 border border-green-200 rounded-lg">
                    <h3 className="text-lg font-medium text-green-900 mb-2">{editingReviewId ? 'Edit your review' : 'Write a review'}</h3>
            ...
            {userReview && (
                <div className="mb-10 p-4 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <p className="text-sm text-gray-500">By {userReview.user?.name} on {formatDate(userReview.createdAt)}</p>
                    <div className="mt-2 flex gap-4 text-sm text-gray-600">
                        <button onClick={() => startEdit(userReview)} className="flex items-center gap-1 hover:text-green-700">
                            <Edit size={14} /> Edit
                        </button>
                        <button onClick={() => handleDelete(userReview._id)} className="flex items-center gap-1 hover:text-red-600">
            ...
            {otherReviews.length === 0 ? (
                ...
                <div>
                    <h3 className="text-lg font-medium text-green-800 mb-4">Other Customer Reviews</h3>
                        ...
                        {otherReviews.map((review) => (
                            <div key={review._id} className="border-b border-green-100 pb-4">
                                ...
                                <p className="text-sm text-gray-500">
                                    By {review.user?.name || 'User'} on {formatDate(review.createdAt)}
                                </p>
                                {isAdmin && (
                                    <div className="mt-2 flex gap-4 text-sm text-gray-600">
                                        <button onClick={() => startEdit(review)} className="flex items-center gap-1 hover:text-green-700">
                                            <Edit size={14} /> Edit
                                        </button>
                                        <button onClick={() => handleDelete(review._id)} className="flex items-center gap-1 hover:text-red-600">
                                            <Trash2 size={14} /> Delete
                                        </button>
                                    </div>
                                )}
